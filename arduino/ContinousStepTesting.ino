...
const int Motor8 = 8;
const int Motor9 = 9;
const int motorSignalPin = 10;  // motor stop/move input

//---------led------------
const int ledsd = 12;
const int ledHioki = 11;

const int homeSignalPin = 13;
...
float resistanceZeroOffsetTM    = 0.0;
...
// ---------------- Motor signal ----------------
int  lastMotorReading       = LOW;
bool motorStopPending       = false;
unsigned long motorStopTime = 0;

// ---------------- App control ----------------
String input                = "";
bool   runArmed             = false;
bool   testExecutedThisStop = false;
...
const unsigned long T_POWER_STABLE     = 500;
const unsigned long T_REED_SETTLE      = 150;
...
void writeToSdCardByStep(String data);
String makeStepFilePath(int stepNum);
bool ensureSdReady();
...
void setup() {
  ...
  lastMotorReading = digitalRead(motorSignalPin);
...
void loop() {
  ...
  if (!runArmed) return;
  if (testRunning) return;   // ignore pin10 while test is running

  int reading = digitalRead(motorSignalPin);

  // Detect rising edge LOW → HIGH (motor just stopped)
  if (reading == HIGH && lastMotorReading == LOW) {
    motorStopPending = true;
    motorStopTime    = millis();
  }

  // Detect falling edge HIGH → LOW (motor moving again)
  if (reading == LOW && lastMotorReading == HIGH) {
    motorStopPending = false;
    motorMovAction();
  }

  // After 100ms delay → read pin13 → decide home or test
  if (motorStopPending && (millis() - motorStopTime >= 100)) {
    motorStopPending = false;
    if (digitalRead(motorSignalPin) == HIGH) {
      motorStopAction();   // ✅ confirmed stop → check pin13 inside
    }
    // if LOW → was noise → ignore
  }
...
// ================================================================
void motorStopAction() {
  if (testExecutedThisStop) return;
  testExecutedThisStop = true;

  // ✅ Read pin13 NOW — only at the moment motor has confirmed stopped
  bool isHome = (digitalRead(homeSignalPin) == LOW);

  if (isHome) {
    // Home position — calibrate only, no test
    step      = 1;
    fileIndex = 1;
    Serial.println("HOME: Taring load cell and calibrating current...");
    LoadCell.start(3000, true);
    LoadCell.setCalFactor(calFactor);
    CalibrateCurrentZero();
    Serial.print("HOME: Current ZERO offset = ");
    Serial.println(currentZeroOffset, 4);
    Serial.println("HOME: Ready. Next stop = Step 1");

    // Advance CoolWorks to next step
    digitalWrite(Motor8, LOW);
    delay(300);
    digitalWrite(Motor8, HIGH);
    delay(100);

    testExecutedThisStop = false;
    return;   // no test at home
  }
  ...
  if (selectedTest == 1 || selectedTest == 2) {
    ...
    Serial.println("No Test Type Selected");
    testExecutedThisStop = false;
  }
}
...
void updateTestSequence() {
  ...
  switch (testState) {
    ...
    case T_SAVE: {
      ...
      line += "Current: " + String(measuredCurrent, 2) + " A,";
      line += "ResTM: ";
    ...
    case T_DONE:
      ...
      // Pulse Motor8 to advance CoolWorks
      digitalWrite(Motor8, LOW);
      delay(300);
      digitalWrite(Motor8, HIGH);
      delay(100);
...
void writeToSdCardByStep(String data) {
  ...
  String path = makeStepFilePath(step);
  ...
  if (file.size() == 0) {
    if (selectedTest == 1)
      file.println("Step,Current(A),ResTM(mOhm),Force(g)");
    else
      file.println("Step,Current(A),ResTM(mOhm),ResOther(mOhm),Force(g)");
  }
...
String makeStepFilePath(int stepNum) {
  char fname[13];
  snprintf(fname, sizeof(fname), "STEP%02d.CSV", stepNum);
  return folderName + "/" + String(fname);
}
...
void processCommand(String cmd) {
  ...
  // ---------- START ----------
  if (cmd == "START") {
    if (!folderReady) { Serial.println("Please Set FileFolder"); return; }
    if (selectedTest == 0) { Serial.println("Please Select Test Mode"); return; }

    doStartSequence();
    delay(500);
    digitalWrite(Motor8, LOW);

    lastMotorReading     = LOW;
    motorStopPending     = false;
    testExecutedThisStop = false;
    runArmed             = true;
    testLocked           = true;

    Serial.print("OK START TESTING OF ");
